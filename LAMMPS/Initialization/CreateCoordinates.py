...
def update_coordinates(loc, coordlist, typelist, iseq, seqstring, isfirst):
...
def create_coordinates(seqstring, datafile):
	...
	#### Write header
	print("ITEM: TIMESTEP", file=file_writer)
	print("0", file=file_writer)
	print("ITEM: NUMBER OF ATOMS", file=file_writer)
	print(natoms, file=file_writer)
	print("ITEM: BOX BOUNDS pp pp pp", file=file_writer)
	print("-500000 500000", file=file_writer)
	print("-500000 500000", file=file_writer)
	print("-500000 500000", file=file_writer)
	print("ITEM: ATOMS id type xu yu zu", file=file_writer)
	...
	if lseq == 1:
	    ...
	    for loc_el in loc:
	        ...
	        icoord += 1
	    coord_mat_old = update_coordinates(loc, coordlist, typelist, iseq, seqstring, 'y')
	    for iseq in range(2, lseq):
	        ...
	        ### update coordinates
	        coord_mat_old = update_coordinates(loc, coordlist, typelist, iseq, seqstring, 'n')
	
	
	    ### Make overall rotation such that line connecting first and last sugar is parallel to x axis
	    pos0 = coordlist[1,0:3]
	    pos1 = coordlist[int(0.5*natoms-1),0:3]
	    vec2 = pos1 - pos0
	    vec2 = vec2/np.linalg.norm(vec2)
	    theta1 = np.arccos(1 - 2*np.random.uniform(0.0, 1.0))
	    phi1 = 2*np.pi*np.random.uniform(0.0, 1.0)
	    #vec1 = np.array([np.sin(theta1)*np.cos(phi1), np.sin(theta1)*np.sin(phi1), np.cos(theta1)])
	    vec1 = np.array([0,0,1])
	...
	for i in range(1,natoms+1):
	    print(str(i)+" "+str(int(typelist[i]))+" "+"{:.4f}".format(coordlist[i,0])+" "+"{:.4f}".format(coordlist[i,1])+" "+"{:.4f}".format(coordlist[i,2]), file=file_writer)
	return coordlist, typelist
